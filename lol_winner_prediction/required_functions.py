...
def show_outliers1(dataframe: pd.DataFrame, col_name: str, q1=0.25, q3=0.75, index=False):
    up, low = outlier_thresholds(dataframe, col_name, q1=q1, q3=q3)
    outliers = dataframe[(dataframe[col_name] > up) | (dataframe[col_name] < low)]

    # Boxplot çizimi
    plt.figure(figsize=(8, 6))
    plt.boxplot(dataframe[col_name], vert=False)
...
def hyperparameter_optimization(X, y, cv=5, scoring="roc_auc", is_classifier=True, is_grid_search=True):
    ...
    for name, model, params in models:
        ...
        if is_grid_search:
            ...
            s_best = RandomizedSearchCV(model, params, n_iter=10, cv=cv, n_jobs=-1, verbose=False).fit(X, y)
...
### by Işıl Hilal AKTAR ###
## https://www.linkedin.com/in/i%C5%9F%C4%B1l-hilal-aktar-8837aa17a/##